/**************************************************************************************************
 * File: scd4x.cpp
 * Description: This file contains the implementation of functions to communicate with the 
 * Sensirion SCD4X CO₂ sensor via I²C protocol. The SCD4X sensor measures CO₂ concentration, 
 ...
 *   - Initialize the sensor and I²C communication
 *   - Read and write commands to the SCD4X
 *   - Perform CRC validation for data integrity
 *   - Retrieve CO₂ concentration, temperature, and humidity data
...
bool SCD4X::SCD4X_ReadCommand(uint8_t* reg_addr, uint16_t* rev_data, uint8_t length)
{
    ...
    int ret = -1 ;
    QThread::msleep(5); // Wait for date every 5ms
    ret = wiringPiI2CRawWrite (SCD4XID, reg_addr , 3);
    if (ret == -1){
        qDebug() << "SCD4XID Command write failure";
        return false;
    }
    else{
...
void SCD4X::SCD4X_WriteCommand(uint8_t* send_data, uint8_t length)
{
    ...
    // qDebug() << "SCD4XID Command write success";
    }
}
...
bool SCD4X::SCD4X_Init(void)
{
    ...
    ret = wiringPiSetup();
    if (ret == -1){
        qDebug() << "WiringPi initialization failure";
        return false;
    }
    else{
